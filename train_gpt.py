...
if __name__ == '__main__':
    ...
    for i in range(checkpoint['iter'], TRAIN_ITER):
        ...
        logger.info(f'logtis: {logtis.shape}') # torch.Size([50, 228, 506])

        probs = logtis[:,:-1,:] # (batch, seq-1, vocab) 去掉最后一个终止符或者padding符号
        logger.info(f'probs: {probs.shape}') # torch.Size([50, 227, 506])
        targets = batch_ids[:,1:] # (batch, seq-1) 去掉最开始的开始符
        logger.info(f'targets: {targets.shape}') # torch.Size([50, 227])
        # print(probs.view(-1, probs.size(2)))
        # print(targets.view(-1))
        logger.info(f'probs.reshape(-1, probs.size(2)): {probs.reshape(-1, probs.size(2)).shape}') # torch.Size([11350, 506]) 拍平的操作之后的形状是 (batch*seq-1, vocab)
        logger.info(f'targets.reshape(-1): {targets.reshape(-1).shape}') # torch.Size([11350]) 拍平的操作之后的形状是 (batch*seq-1)
